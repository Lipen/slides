...
#metro-setup(group-separator: sym.space.thin)
...
#let slide(title: none, is_toc: false, body) = {
  ...
  let header = context {
    ...
    if title != none {
      ...
      if not is_toc {
        text(fill: section-foreground, size: 0.6em, last-section.get())
        text([\ ])
      }
  ...
  let footer = context {
    ...
    text(fill: footer-color, custom-footer.get())
    h(1fr)
...
#let focus-slide(body) = {
  set page(fill: focus-background, margin: (x: 2em, y: 1em))
